...
- **FR14**: コインはフィールド上のランダムな位置に配置される。
  配置枚数と、そのうち特殊コインにする枚数は `GameStateSettings` で指定する。
  「どこに何を置くか」の判断は `ICoinPlacementGenerator` として MonoBehaviour から分離し、
  実装を差し替えることで配置ルールを変更できる。
  - 検証: `RandomCoinPlacementGenerator` の EditMode テストで、指定枚数・特殊コインの枚数・
    フィールド内に収まること・指定した高さ・同一シードでの再現性・最小距離の確保を確認する。
    あわせて PlayMode で 30 枚が重ならずに配置されることを目視確認する。（EditMode は実装済み）
  - 情報源: 利用者からの依頼（2026-08-17）

- **FR15**: プレイヤーがコインに接触すると、通常コインは取得枚数を増やし、
  特殊コインは加えて無敵時間を設定値まで戻す。取得したコインはシーンから消える。
  接触判定はタグや名前ではなく、プレイヤーの `Collider` の参照比較で行う。
  - 検証: PlayMode で通常コインを取得して枚数が増えること、
    特殊コインを取得して無敵状態になることを確認する。（未実行）
  - 情報源: 利用者からの依頼（2026-08-17）
