...
## ADR-012: コインの配置ルールを `ICoinPlacementGenerator` として分離する

### Context

利用者から通常コイン・特殊コインのプレハブが提供され、ランダム配置の実装を依頼された
（2026-08-17）。本来 `INT-001` の対象外だったが、`GameState` 側の受け口
（`CollectCoin` / `CollectSpecialCoin`）は ADR-011 で既に用意してあった。

### Decision

移動ロジック（ADR-003）と同じ切り分けを適用する。

- **Core 層**: `ICoinPlacementGenerator` が「どこに何を置くか」だけを決め、
  `IReadOnlyList<CoinPlacement>` を返す。`RandomCoinPlacementGenerator` が
  最小距離を保つ棄却サンプリングで実装する。
  `System.Random` を外から受け取るため、シードを固定すれば配置を再現できる。
- **Unity 層**: `CoinView` は「トリガーに触れられたら渡されたコールバックを呼んで自壊する」
  だけを担当する。**自分が通常コインなのか特殊コインなのかを知らない。**
- **組み立て**: `PlayCompositionRoot` が配置結果に応じてプレハブを実体化し、
  取得時の振る舞い（枚数を増やす／無敵にする）をコールバックとして渡す。

接触判定はタグや名前ではなく、`CoinView.Initialize` に**プレイヤーの `Collider` の実物**を
渡して参照比較する。どのオブジェクトを「プレイヤー」とみなすかを組み立て役が明示的に決められる。

### Consequences

- 配置ルールが GameObject の生成から独立するため、シーンを開かずに EditMode テストできる。
  枚数・特殊コインの数・フィールド内・高さ・シード再現性・最小距離を検証する。
- 実装を差し替えれば格子状配置や手動配置に変更できる。
  `ICharacterMoveLogic` と同じ差し替えの型がもう 1 つ増え、教材として反復になる。
- 枚数に対してフィールドが狭いと最小距離を保てない。
  50 回試して駄目なら重なりを許容する設計とし、無限ループを避けている。
- `CoinView` が種類を知らないため、「特殊コインだけ挙動を変える」処理が
  `PlayCompositionRoot` の 1 箇所に集まる。
- タグ・レイヤーという文字列ベースの暗黙的な結び付きを持ち込まずに済む。

### Alternatives Rejected

- **`CoinView` に `CoinKind` を持たせて自分で分岐する**: 却下。
  ビューが状態の意味を知ることになり、`CharacterView` が移動ロジックを知らない方針と食い違う。
- **タグ `Player` で接触相手を判定する**: 却下。
  文字列による暗黙の結び付きになり、依存が明示されない。
- **配置を `PlayCompositionRoot` に直接書く**: 却下。テストできなくなる。

### トレーサビリティ

FR14、FR15、ADR-003、ADR-007、ADR-011
